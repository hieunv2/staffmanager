      ...
      <table :class="['vuetable', css.tableClass, css.tableHeaderClass]">
        <thead>
          <tr>
            ...
            <th v-if="scrollVisible" :style="{width: scrollBarWidth}" class="vuetable-gutter-col" />
          </tr>
        </thead>
      </table>
    </div>
        ...
        <tbody v-cloak class="vuetable-body">
          <template v-for="(item, itemIndex) in tableData">
            ...
            <template v-if="useDetailRow">
              <Transition :key="itemIndex" :name="detailRowTransition">
                <tr
                ...
                >
                  <td :colspan="countVisibleFields">
                    <Component :is="detailRowComponent" :row-data="item" :row-index="itemIndex" />
                  </td>
                </tr>
              </Transition>
            </template>
          ...
          <template v-if="displayEmptyDataRow">
            <tr>
              <td :colspan="countVisibleFields" class="vuetable-empty-result" v-html="noDataTemplate" />
            </tr>
          </template>
          <template v-if="lessThanMinRows">
            <tr v-for="i in blankRows" :key="i" class="blank-row">
              <template v-for="(field, fieldIndex) in tableFields">
...
&nbsp;
  ...
  <table v-else :class="['vuetable', css.tableClass]">
            ...
            <template v-if="field.visible">
              ...
              <template v-else>
                ...
                <td
                  ...
                  @dblclick="onCellDoubleClicked(item, field, $event)"
                  @contextmenu="onCellRightClicked(item, field, $event)"
                  v-html="getObjectValue(item, field.name, '', field.replace)"
                />
              </template>
            </template>
          </template>
  ...
  methods: {
      ...
      this.fields.forEach(function(field, i) {
        if (typeof (field) === 'string') {
          ...
          obj = {
            ...
            props: (field.props === undefined) ? {} : field.props,
    ...
    renderTitle(field) {
      ...
      if (title.length > 0 && this.isInCurrentSortGroup(field) || this.hasSortableIcon(field)) {
        const style = `opacity:${this.sortIconOpacity(field)};position:relative;`
    ...
    fetch(apiUrl, httpOptions) {
      return this.httpFetch
        ? this.httpFetch(httpOptions.params)
    ...
    isSortable(field) {
      return !(typeof field.sortField === 'undefined' && field.sort)
    },
    ...
    orderBy(field, event) {
      ...
      if (field.sortField.indexOf(',') >= 0) {
        const childFields = field.sortField.split(',')
				childFields.forEach(f => this.multiColumnSort({ ...field, sortField: f }))
      } else
    ...
    checkCheckboxesState(fieldName) {
      ...
      const self = this
      const idColumn = this.trackBy
      const selector = 'th.vuetable-th-checkbox-' + idColumn + ' input[type=checkbox]'
